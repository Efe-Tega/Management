...
-----------------------------------*/
  ...
  $searchPriceRange
    .find(".ui-slider-handle:eq(0)")
    .html("<span>" + "$" + $searchPriceRange.slider("values", 0) + "</span>");
  $searchPriceRange
    .find(".ui-slider-handle:eq(1)")
    .html("<span>" + "$" + $searchPriceRange.slider("values", 1) + "</span>");
...
-----------------------------------*/
  ...
  $(".dropzone").dropzone({
    url: "/file/post",
    dictDefaultMessage:
      "<i class='pe-7s-cloud-upload'></i> Click here or drop files to upload",
  });
...
-----------------------------------*/
  ...
  $addPropertyTabNav.on("click", "a", function () {
    ...
    var $addPropertyTabList = $this
      .closest(".add-property-wrap")
      .find('a[href="' + $thisTarget + '"]')
      .parent("li");
...
-----------------------------------*/
  ...
  $registerToggle.on("click", function (e) {
    ...
    var $this = $(this);
    var $thisTarget = $this.attr("href");
    var $loginRegisterTabList = $this
      .closest("#main-wrapper")
      .find('a[href="' + $thisTarget + '"]');
    var $loginRegisterTabPane = $this
      .closest("#main-wrapper")
      .find(".tab-pane");

    $loginRegisterTabList.addClass("active");
    $loginRegisterTabList
      .parent("li")
      .prevAll("li")
      .find("a")
      .removeClass("active");
